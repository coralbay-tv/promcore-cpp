#include "promcore/text_serializer.h"
...
void WriteValue(std::ostream& out, const double value) {
  if (std::isnan(value)) {
    out << "Nan";
...
void WriteValue(std::ostream& out, const int64_t value) {
    out << value;
}
...
void SerializeCounter(std::ostream& out, const MetricFamily& family,
  ...
  const auto &counter{metric.counter};
  if (counter.type == ClientMetric::Counter::Type::DOUBLE) {
    WriteValue(out, counter.value.d);
  }
  else {
    WriteValue(out, counter.value.i);
  }
...
void SerializeGauge(std::ostream& out, const MetricFamily& family,
  ...
  if (gauge.type == ClientMetric::Gauge::Type::DOUBLE) {
    WriteValue(out, gauge.value.d);
  }
  else {
    WriteValue(out, gauge.value.i);
  }
  WriteTail(out, metric);
}
