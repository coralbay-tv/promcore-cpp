...
#include "promcore/collectable.h"
...
/// a data race.
class PROMCORE_CPP_CORE_EXPORT Registry : public Collectable {
 public:
 ...
 private:
  ...
  std::vector<std::unique_ptr<Family<CounterDouble>>> countersDouble_;
  std::vector<std::unique_ptr<Family<CounterInt>>> countersInt_;
  std::vector<std::unique_ptr<Family<GaugeDouble>>> gaugesDouble_;
  std::vector<std::unique_ptr<Family<GaugeInt>>> gaugesInt_;
