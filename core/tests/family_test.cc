...
TEST(FamilyTest, labels) {
  ...
  Family<CounterInt> family{"total_requests",
                         "Counts all requests",
...
TEST(FamilyTest, reject_same_label_keys) {
  ...
  Family<CounterInt> family{"total_requests", "Counts all requests", labels};
  EXPECT_ANY_THROW(family.Add(labels));
}

TEST(FamilyTest, counter_value_double) {
  Family<CounterDouble> family{"total_requests", "Counts all requests", {}};
  auto& counter = family.Add({});
  counter.Increment();
  const auto collected = family.Collect();
  ASSERT_GE(collected.size(), 1U);
  ASSERT_GE(collected[0].metric.size(), 1U);
  EXPECT_EQ(ClientMetric::Counter::Type::DOUBLE, collected[0].metric.at(0).counter.type);
  EXPECT_EQ(1, collected[0].metric.at(0).counter.value.d);
}

TEST(FamilyTest, counter_value_int) {
  Family<CounterInt> family{"total_requests", "Counts all requests", {}};
  auto& counter = family.Add({});
  counter.Increment();
  const auto collected = family.Collect();
  ASSERT_GE(collected.size(), 1U);
  ASSERT_GE(collected[0].metric.size(), 1U);
  EXPECT_EQ(ClientMetric::Counter::Type::INT, collected[0].metric.at(0).counter.type);
  EXPECT_EQ(1, collected[0].metric.at(0).counter.value.i);
}
...
TEST(FamilyTest, throw_on_invalid_metric_name) {
  auto create_family_with_invalid_name = []() {
    return detail::make_unique<Family<CounterInt>>("", "empty name", Labels{});
  };
...
TEST(FamilyTest, throw_on_invalid_constant_label_name) {
  auto create_family_with_invalid_labels = []() {
    return detail::make_unique<Family<CounterInt>>(
