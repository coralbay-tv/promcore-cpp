...
class TextSerializerTest : public testing::Test {
 public:
  std::string Serialize(MetricType type) const {
    ...
    metricFamily.metric = std::vector{metric};

    std::vector families{metricFamily};
...
TEST_F(TextSerializerTest, shouldSerializeNotANumber) {
  metric.gauge.type = ClientMetric::Gauge::Type::DOUBLE;
  metric.gauge.value.d = std::nan("");
...
TEST_F(TextSerializerTest, shouldSerializeNegativeInfinity) {
  metric.gauge.type = ClientMetric::Gauge::Type::DOUBLE;
  metric.gauge.value.d = -std::numeric_limits<double>::infinity();
...
TEST_F(TextSerializerTest, shouldSerializeTimestamp) {
  metric.counter.type = ClientMetric::Counter::Type::DOUBLE;
  metric.counter.value.d = 64.0;
